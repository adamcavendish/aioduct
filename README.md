...
```toml
[dependencies]
aioduct = { version = "0.2.0-alpha.4", features = ["tokio"] }
```
...
```toml
aioduct = { version = "0.2.0-alpha.4", features = ["tokio", "rustls", "rustls-ring"] }
```

To use rustls with AWS-LC instead of ring, select the AWS-LC provider:

```toml
aioduct = { version = "0.2.0-alpha.4", features = ["tokio", "rustls", "rustls-aws-lc-rs"] }
```

To use the OS certificate store, add `rustls-native-roots` alongside either TLS provider:

```toml
aioduct = { version = "0.2.0-alpha.4", features = ["tokio", "rustls-native-roots", "rustls-aws-lc-rs"] }
```
