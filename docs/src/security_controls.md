...
RFC 9421 HTTP Message Signatures are not yet built in. Callers can still add precomputed `Signature` and `Signature-Input` headers through request builders or middleware. Built-in canonicalization and signing helpers are Future Work.
