...
| HTTP Message Signatures | `message_signatures` builds RFC 9421 request signature bases and formats `Signature-Input` / `Signature` headers for caller-provided signature bytes. |
...
RFC 9421 request signature-base generation is available through `MessageSignatureConfig`. Callers choose the signing algorithm, sign the generated base, then attach the formatted `Signature-Input` and `Signature` headers. Native automatic signing, response verification, `Accept-Signature`, multiple-signature merging, and automatic body digest generation remain Future Work.
