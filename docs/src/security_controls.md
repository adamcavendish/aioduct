...
| HTTP Message Signatures | `message_signatures` builds RFC 9421 request signature bases, formats `Signature-Input` / `Signature` headers for caller-provided signature bytes, applies request verification policy checks, and can automatically sign finalized native request attempts. |
