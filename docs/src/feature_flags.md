...
aioduct = { version = "0.2.3", features = ["tokio", "rustls", "rustls-ring", "json"] }
...
aioduct = { version = "0.2.3", features = ["smol"] }

# HTTPS, smol runtime
aioduct = { version = "0.2.3", features = ["smol", "rustls", "rustls-ring"] }

# HTTP only, compio runtime (experimental)
aioduct = { version = "0.2.3", features = ["compio"] }

# HTTPS + JSON + compression, tokio runtime
aioduct = { version = "0.2.3", features = ["tokio", "rustls", "rustls-ring", "json", "gzip", "brotli", "zstd", "deflate"] }

# Blocking client (synchronous)
aioduct = { version = "0.2.3", features = ["tokio", "rustls", "rustls-ring", "blocking"] }

# With tracing and OpenTelemetry
aioduct = { version = "0.2.3", features = ["tokio", "rustls", "rustls-ring", "tracing", "otel"] }

# With tower integration
aioduct = { version = "0.2.3", features = ["tokio", "rustls", "rustls-ring", "tower"] }

# Hickory DNS resolver
aioduct = { version = "0.2.3", features = ["tokio", "rustls", "rustls-ring", "hickory-dns"] }

# DNS-over-HTTPS
aioduct = { version = "0.2.3", features = ["tokio", "rustls", "rustls-ring", "doh"] }

# DNS-over-TLS
aioduct = { version = "0.2.3", features = ["tokio", "rustls", "rustls-ring", "dot"] }
...
aioduct = { version = "0.2.3", features = ["wasmtime", "smol", "rustls", "rustls-ring"] }

# Wasmtime host adapter with compio + rustls/ring
aioduct = { version = "0.2.3", features = ["wasmtime", "compio", "rustls", "rustls-ring"] }
```
