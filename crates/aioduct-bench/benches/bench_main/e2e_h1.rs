...
fn bench_h1_get(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("e2e_h1/get_small");
...
fn bench_h1_get_text(c: &mut Criterion) {
    let url = h1_small_url();
    let mut group = c.benchmark_group("e2e_h1/get_text");
    group.bench_function("aioduct", |b| {
        b.to_async(&*RT).iter(|| async {
            AIODUCT_H1
                .get(&url)
...
fn bench_h1_post_4k(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("e2e_h1/post_4k");
    group.bench_function("aioduct", |b| {
        b.to_async(&*RT).iter(|| {
            let p = payload.clone();
            async {
...
fn bench_h1_download_64k(c: &mut Criterion) {
    let url = h1_64k_url();
    let mut group = c.benchmark_group("e2e_h1/download_64k");
...
fn bench_h1_download_1m(c: &mut Criterion) {
    let url = h1_1m_url();
    let mut group = c.benchmark_group("e2e_h1/download_1m");
